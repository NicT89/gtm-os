...
| 14 | Sequence update endpoint, on a live sequence containing an empty-subject template | — | **Cannot round-trip its own state.** Resending the sequence exactly as it currently stands, with no additions, is rejected: *"Please make sure your template has content before turning it on."* Several legitimate templates carry an empty subject — replies in a thread, and AI-variable bodies — which the UI creates happily and the update validator refuses when `active` is true. `active` is a required parameter, so the only way to satisfy the validator is to deactivate a live sequence. | Before planning any API edit, send the current state back unchanged as a no-op. If that is rejected, the sequence is effectively read-only through the API and the edit belongs in the UI. Rejections are atomic — three failed attempts left all steps, touches, statuses and templates intact. |
