...
This skill needs the CRM (`{CRM_PROVIDER}`) to match and create records, and the Research Vault to log the run. Without the Vault keys, produce the matched/created records and the summary but skip the Run row and say so. Without the CRM, stop: there is nothing to attribute against. Route the user to the `environment-setup` skill (the plugin root's `references/environment-setup.md`); not set up almost never means not owned.
...
4. **Net-new records**: create with the event as original source, tagged per the house naming convention: everything the engine creates carries "[Claude]" in its name, and human-managed records are never edited. State the count and any credit cost BEFORE creating; bulk creation is a gated spend.
